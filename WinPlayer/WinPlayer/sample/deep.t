...
suffowarn2: function;
airLight: function;
tram2Planet: function;
tram2Hell: function;
/* tram2Station: function; */
shipDie: function;
distressCall: function;
hasMonster: function;
isTubeDest: function;
shuttleDaemon: function;
setTubeNose: function;
vacDaemon: function;
gauntletRobotFuse: function;
bombDaemon: function;
escapeDaemon: function;
exitOptions: function;
goToSleep: function;
caveDaemon: function;
stationShake: function;
dropAll: function;
...
getGameInfo: function
{
    return
        ...
        'Byline', 'by Michael J. Roberts and Steve McAdams',
        'Desc', 'A classic science fiction text adventure.  Stranded
                after running out of fuel, you must try to get help
                from a nearby space station, but the place is abandoned
                and strange things seem to have been happening lately.
                Find a way down to the planet if you dare.',
...
stationShake: function( parm )
{
  local severity;

  if ( not Me.isasleep )       // suppress explosion while dreaming
  {
    "\b";
    if ( not Me.location.isvac )
    {
        severity := rand( 10 );
        if ( severity = 1 )
            "The station shakes slightly, as though some large object hit the
            exterior. ";
        else if ( severity < 4 )
            "A loud crashing sound explodes all around you. The whole room
            shakes violently for several moments. The shaking eventually
            subsides. ";
        else if ( severity < 10 )
            "A terrifying roar suddenly rips through the station, as though
            a huge explosion had just happened nearby. The floor buckles and
            the room seems to spin. For long moments, the room rumbles and
            shakes. Eventually, the shaking subsides, and you take a moment
            to regain your balance. ";
        else
        {
            "The room is suddenly jolted with a huge shock. The lights go out,
            and the whole station seems to spin and flip end over end. You
            reach in vain for something to hold on to, but the whole room seems
            to be spinning. After a few moments, the lights come back on, and
            the station seems to recover normal gravity; the rumbling and
            shaking continue for several more moments, the structure of the
            station creaking and moaning the whole time. Eventually, things
    ...
    {
        "There is a brilliant flash of light, and the station shakes
        violently. You don't know how, but somehow you manage to avoid
        floating out into space. After a few moments, the shaking stops. ";
    }
  }
...
/*
 *   commonInit - perform common initializations that we want to do
 *   regardless of whether we're starting with a saved game or not 
 */
commonInit: function
{
...
init: function
{
    ...
    "\b\b\bIt was a more exciting time, back in the early years of deep space
    exploration. Anyone who could afford a good ship and a tank full
    of fuel could strike out into the vast empty reaches to find his
    fortune among the stars.
    \bOf course, you'd have a better chance of finding your fortune
    if you had remembered to check a nav chart before heading off
    into the void. Now you're nearly out of fuel and air, you're
    tired and hungry -- and desperately hoping you can find someone
    monitoring the distress channels.
    \b"; version.sdesc;
    ...
    global.sleepList := global.sleepList + vacRobot;
    startBelt.isfastened := true;
    startChair.fastenitem := startBelt;
    Me.location := startChair;
    Me.cantSleep := true;
    startChair.lookAround( true );

    setversion( 'Deep Space Drifter V' + version.vsnnum );

    global.dreamList := global.dreamList + swampDream + xeniteDream;
...
preinit: function
{
    ...
    l := [];
    o := firstobj(depotRoom);
    while(o <> nil)
    {
        if (o.isdepot) l := l + o;
        o := nextobj(o, depotRoom);
    }
    global.depotList := l;
...
shipland: function( parm )
{
    "\bA sudden shock reverberates through your ship. \"Warning,\" the
...
stopCycle: function( parm )
{
    if (( not stationAir.iscycled ) and cycleButton.armState = 1 )
    {
        if ( Me.location = stationAir )
...
suffowarn: function( parm )
{
    if ( Me.location.isvac )
    {
        "\n\t";
        if ( suitTank.location = spacesuit )
        {
            "Your air tank doesn't seem to be working too well.
            You did check that it was full before you attempted
            extra vehicular activity, didn't you? ";
        }
        else
        {
            "When going into a vacuum environment, you recall
            from your Pilot Ed class back in high school,
            always bring along an oxygen tank. Which you
            forgot to do. Consequently, you are getting quite
            short on air. ";
        }
        setfuse( suffowarn2, 2, nil );
    }
}
suffodo: function( parm )
{
    if ( Me.location.isvac )
    {
        "\n\tYou pass out from lack of oxygen, but not until
        you've had the opportunity to fog up your helmet's
        faceshield with your industrious panting and carrying
        on. ";
        remdaemon( airLight, nil );
        die();
    }
}
suffowarn2: function( parm )
{
    if ( Me.location.isvac )
    {
...
airLight: function( parm )
{
    if ( Me.location.isvac )
    {
        "\n\tA red indicator light in your helmet depicting
        an empty air tank is flashing insistently";
        global.suffoCount := global.suffoCount + 1;
        if ( global.suffoCount > 4 )
            ", accompanied by an ear-piercing alarm klaxon. ";
        else ". ";
    }
    else remdaemon( airLight, nil );
}
tram2Planet: function( parm )
{
    local l;
    l := tramShip.launchState;
    if ( l = 2 )
        "\bThe retros fire gently. The tram slowly loses its orbital
        velocity, and starts to fall toward the planet.
        \n\tThrough your viewport, you see a giant beam of energy from
        the planet, which seems to be aimed directly at your tram! After
        a moment, though, you realize that the beam is just missing your
        tram.  It is, however, directly hitting the space station.
    ...
    {
        "\bThrough
        the viewscreen you can see nothing more than the
        bright red of incandescing air as the tram plunges
        ...
        {
            "Luckily you are securely buckeled in, and
            ride out this part of the trip feeling only
            slightly nauseous. ";
        }
        else
        {
            "Unfortunately, you've neglected to attach yourself
        ...
        "\bThe tram starts firing its landing rockets. ";
    else if ( l = 5 )
        "\bThe tram gently settles onto the ground. The
        motors slowly power down. ";
    else if ( l = 6 )
    {
        "\bThe hatch slides open. Outside, you hear giggling. You
...
tram2Hell: function( parm )
{
    local s;

    s := tramShip.launchState;
    if ( s < 105 )
        "\bThe tram continues to drift into space. ";
    else
    {
        "\bThe tram inexplicably decides to fire its engines
...
/*
tram2Station: function( parm )
{
    local s;
    s := tramShip.launchState;
    if ( s = 9 )
        "\bThe rockets cut off. The tram leaves the last remnants
        of atmosphere. ";
    else if ( s = 10 )
        "\bThe space station approaches in the distance. The retros
        fire. ";
    else if ( s = 11 )
        "\bThe Tram is slowly drifting toward the station. ";
    else if ( s = 12 )
    {
        tramShip.launchState := 0;
        tramHatch.isopen := true;
        "\bThe tram approaches one of the station's docking ports,
        and finally comes up against it, lurching to a halt.
        You hear the motors power down. After a few moments, the
        hatch slides open. ";
        if ( not tramShip.hasScored )
        {
    ...
    if ( s <> 12 ) tramShip.launchState := s + 1;
}
*/
shipDie: function( parm )
{
    if ((( Me.location = startroom ) or ( Me.location = startChair )) and
           not ( startroom.isdocked ))
    {
        "\bYour ship finally runs out of air. You run out soon
        thereafter. ";
        die();
    }
}
distressCall: function( parm )
{
    "\bYour computer's tinny voice announces, \"Now receiving
...
hasMonster: function( rm )
{
    if ( monster1.location=rm ) return( monster1 );
    if ( monster2.location=rm ) return( monster2 );
    return( nil );
}
isTubeDest: function( r )
{
    if ( r.outTubeX = shuttle.xcoord and
     r.outTubeY = shuttle.ycoord and
     r.outTubeZ = shuttle.zcoord )
    {
        shuttle.tubeDest := r;
        return( true );
    }
...
shuttleDaemon: function( parm )
{
    local x, y, thereYet, i, cnt, l, found;

    if ( shuttle.outRem>1 )       // outbound from station
    {
        if ( Me.location = shuttle ) "\bThe shuttle speeds down the tube. ";
        shuttle.outRem := shuttle.outRem - 1;
    }
    else if ( shuttle.outRem=1 )  // emerging into central
    {
        shuttle.outRem := 0;
        shuttle.moveInto( centralHub );
        if ( Me.location = shuttle )
            "\bThe shuttle emerges into a gigantic hub,
    ...
    else if ( shuttle.inRem>1 )   // inbound to station
    {
        if ( Me.location = shuttle ) "\bThe shuttle speeds down the tube. ";
        shuttle.inRem := shuttle.inRem - 1;
    }
    else if ( shuttle.inRem=1 )   // arriving at station
    {
        shuttle.inRem := 0;

        if ( shuttle.tubeDest = tubeRubble )
        {
            setTubeNose( tubeRubble );
            if ( Me.location = shuttle )
                "\bThe shuttle slows to a stop. You notice that the tube
        ...
        else
        {
            ...
            if ( Me.location = shuttle )
            {
                "\bThe shuttle emerges into a depot and comes to a stop.";
                if ( not canopy.isblown ) " The canopy opens.";
                "\b";
                shuttle.location.enterRoom( Me );
            }
            else if ( Me.location = shuttle.location )
                "\bA shuttle emerges from the tunnel and comes to a stop. ";
